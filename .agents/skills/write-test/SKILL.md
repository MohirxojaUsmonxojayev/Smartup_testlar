...
- Smartup test yozish jarayonida yangi formaga kirilganda yoki URL/form state o'zgarganda screenshotni `test-results/screens/smartup/<url_as_filename>.png` ko'rinishida saqla; keyingi muammoda avval shu screenshot arxividan foydalan.
...
- Mexanizm pytest hook/marker orqali bo'lsin: bitta group testi failed bo'lsa faqat shu groupning keyingi testlari skip qilinadi, boshqa group markerlari skip qilinmaydi; user_setup failed bo'lsa barcha group testlar skip qilinadi.
- Grouplar bir-birining browser/page holatini meros qilib olmasin: group runnerlarda alohida group page/context yoki kamida har testda fresh page ishlatish kerak.
...
- `test_smoke_runner.py` ichidagi mavjud barcha testlar user setup testlari hisoblanadi; ular yozib bo'lingan.
...
- A-group contract testida `contract_code` bilan birga `contract_name` ham `data_store.json` ga saqlanadi; order formasidagi `Договор` maydoni contract code emas, contract name bilan tanlanadi.
